...
报告生成后，主流程由前端展示可切换背景的总结阅读卡，适合手机直接截图分享，不依赖服务器生成图片。原有 SVG 接口仍保留作兼容和调试用途：

```text
GET /api/v1/sessions/{sessionId}/summary.svg
```

图片优先使用 LLM 总结；没有配置 LLM 时使用本地草稿和用户标记。SVG 由程序排版，适合中文文字，不依赖图片模型，也不会出现中文文字乱码。该接口不是手机端主流程。

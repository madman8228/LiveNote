...
function reportStatusNote(report: ReportResponse | undefined): string {
  if (!report || report.summaryStatus === 'GENERATED') return ''
  if (report.summaryStatus === 'NOT_CONFIGURED') return '当前显示的是本地抽取式草稿，不是语义模型总结。配置服务器 LLM 后重新处理，才能生成主题、知识点和知识结构。'
  if (report.summaryStatus === 'FAILED') return '语义总结未成功，下面保留 ASR 和本地草稿；请先查看错误信息，再重新处理。'
  return '当前没有可用的语义总结，下面显示本地草稿。'
}
